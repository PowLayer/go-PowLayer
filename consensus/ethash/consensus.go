...
// Uncle rewards are disabled (set to zero).
func accumulateRewards(config *params.ChainConfig, state *state.StateDB, header *types.Header, uncles []*types.Header) {
	blockNumber := header.Number.Uint64()
	blockReward := big.NewInt(0) // default case: 0 reward

	switch {
	case blockNumber <= 500_000:
		// Blocks 1 – 500,000: 1 coin per block
		blockReward = big.NewInt(1_000_000_000_000_000_000) // 1e18
	case blockNumber <= 1_500_000:
		// Blocks 500,001 – 1,500,000: 0.5 coin per block
		blockReward = big.NewInt(500_000_000_000_000_000) // 0.5e18
	case blockNumber <= 71_500_000:
		// Blocks 1,500,001 – 71,500,000: 0.1 coin per block
		blockReward = big.NewInt(100_000_000_000_000_000) // 0.1e18
	}

	// Only the block miner gets the reward, no uncle payouts
